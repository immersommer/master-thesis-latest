...
Quark containers~\cite*{quark} is an OCI-compliant container runtime written in the Rust language. It shares a similar architecture with the Kata container, ensuring an equivalent level of container isolation. The differentiating factor lies in the guest OS used in the Quark: a minimal OS that 
eliminates redundant code, in contrast to the Linux kernel used by the Kata container. Consequently, Quark exhibits a smaller memory footprint, faster boot time, and higher performance~\cite*{quark_performance_report}.
...
management, memory management, file system, and network stack. Notably, Qkernel utilizes the host operating system and Qvisor to manage physical devices, thereby avoid sophisticated IO handling and physical memory management. In addition, Qkernel implements most of the Linux system calls. Thus, The 
guest process can trigger a system call using the X86 syscall instruction. Note that this call may be redirected by Qkernel to the host through the Hypercall, Qcall, or Ucall interfaces. To accept the request from the Quark shim,  Qkernel implements a socket server called Quark agent. It uses Ucall 
...
The host operating system is responsible for performing IO read and write operations on physical devices. Therefore, Quark introduces the Ucall mechanism for communicating with the host kernel. This mechanism utilizes the Linux io\_uring and shared queues between Quark and Qkernel. It supports
...
Hardware-based TEE models can be categorized into two main types: virtual machine-based TEE and process-based TEE~\cite*{10.3389/fcomp.2022.930741}. Examples of virtual machine-based TEEs include Intel TDX~\cite*{Inte_TDX} and AMD SEV SNP~\cite*{SEV_SNP_white_book}, whereas Intel SGX~\cite*{INTEL_SGX} is the most famous process-based TEE. 
