...
Figure ~\ref{fig:quark_design}  illustrates the overview of Quark. Quark features an architecture similar to the classic qemu/KVM VM, where qvisor acts as the VM manager and Qkernel serves as the guest kernel 
...
As can be seen in Figure ~\ref{fig:quark_io} , all system calls issued by containerized applications are forwarded to the qkernel through the Guest System Call interface, i.e., X86-64 SysCall/SysRet. 
...
\todo{describe when hyper call instead of qcall is used}
Hypercall provides the qkernel with the means to issue commands to the qvisor. With hypercall, the qkernel can request service in qvisor, such as opening a file. However, 
using hypercall is very expensive due to the context switching between the qkenel and qvisor. To this end, Quark provides an asynchronous mechanism called Qcall as an alternative 
to Hypercall. This mechanism is implemented by a queue on shared memory between the qkernel and the qvisor and an IO thread on the qvisor side, where the shared queue is used to transfer requests 
sent by the qkernel to the qvisor, and the IO thread is responsible for retrieving requests from the queue and notifying the qkernel thread when the qivosr completes 
...
\todo{add picture to show how ucall works}
...
an IO thread in the Host space, and the Linux IO Uring. Qkernel sends IO requests through the guest submission queue. The IO thread on the host side scans the guest submission 
