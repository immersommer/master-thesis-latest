...
The shield offers two modes: Production and Development\todo{new font}. As illustrated in Listing~\ref{lst:shiled_policy_measyrement}, the application owner can determine the mode by utilizing the \emph{shield\_mode} keyword in the shield policy. In Production\todo{new font} mode, the reference values in the shield policy will be used for checking the correctness 
of the shield startup hash and runtime hashes obtained from the software measurement manager. To get these reference values, the application owner should set the \emph{shield\_mode} to Development\todo{new font} and run \acrshort{CVM} in a trusted environment. 
...
call ID. However, before it does so, it must call the interceptor. The interceptor decides whether the system call is allowed based on its configuration in the shield policy. If the system call is not permitted, the guest system call entry point returns EPERM\todo{new font} instead of calling the 
system call handler. 

The interceptor's configuration is shown in Listing~\ref{lst:shiled_policy_interceptor}. The keyword \emph{enable} determines whether the interceptor is enabled. The keyword \emph{mode} specifies the system interceptor's mode: ContextBased or GlobalBased\todo{new font}. In GlobalBased\todo{new font} mode, the interceptor 
functions across all guest processes, while in ContextBased\todo{new font} mode, it operates exclusively for the application process. Considering the potential presence of multiple processes in the guest user space, it is strongly recommended to employ GlobalBased mode\todo{new font}. Currently, the system 
interceptor supports only the action, ScmpActErrno\todo{new font}, which entails rejecting system calls when their IDs are absent from the allowlist termed \emph{syscalls}. Additionally, since the shield policy is securely deployed into the \acrshort{CVM} along with the application secrets, 
...
the Qkernel configuration file~\cite*{quark_conf_file}. Furthermore, the keyword \emph{allowed\_max\_log\_level} designates the maximum log level Qkernel can print. As discussed in Section~\ref{sec:VM_Misconfiguration}, Qkernel supports five log levels. For instance, when \emph{allowed\_max\_log\_level} is set to INFO\todo{new font}, Qkernel is exclusively 
restricted to printing logs at the INFO\todo{new font}, and ERROR\todo{new font} levels. Notably, the Qkernel log manager is enabled and employs the log level OFF\todo{new font} before the \acrshort{CVM} obtains the policy from the secret manager.
