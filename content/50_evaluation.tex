...
% \begin{longtable}{  p{3.4cm}  p{3.4cm}  p{3.4cm} p{2cm} }
%   \caption{\centering DUSP Protein or RNA expression in animal and human brain\label{long3}}
%         \toprule
...
%   \tiny
%   \centering
%   \begin{tabular}{  p{5cm}  p{5cm}  p{3.4cm} }
%     \toprule
%     \multicolumn{3}{c}{Classification of the criticel .} \\[.5\normalbaselineskip]
%     Vulnerabilities   & Solution  & Comment \\
%     \midrule
%     1.~Untrusted runtime manage and deploy secret & Offload the secret management and deployment to secret manager and shielding layer &  Mitigated (Section 5.1) \\[.5\normalbaselineskip]
%     2.~EXEC operation \\
%     \tabitem Lack of authentication and access control & Add EXCE checker to Qkernel &  Mitigated (Section 5.1) \\
%     \tabitem  Missing protection for commands transmitted by untrusted runtime. & Add cryptographic protection to privileged commands &  Mitigated (Section 5.1) \\[.5\normalbaselineskip]
%     3.~Lack protection for STDIO of processes & Add STDIO shield in guest kernel   &   \tabitem Application logs, command execution results are encrypted （Section 5.2） \\ 
...
%     Vulnerabilities   & Solution  & Comment \\
%     \midrule
%     1.~Untrusted runtime manage and deploy secret  & Offload secret management and deployment to secret manager & mitigated (Section 5.1)\\[.5\normalbaselineskip]
%     2.~EXEC operation \\
%     \tabitem Lack of authentication and access control  & Add EXCE checker to Qkernel &  mitigated (Section 5.1) \\[.5\normalbaselineskip]
%     \tabitem Missing protection for commands transmitted by untrusted runtime.  & Add cryptographic protection to privileged commands & mitigated (Section 5.1)\\[.5\normalbaselineskip]
%     3.~Lack protection for STDIO of processes & Add STDIO shield in guest kernel   &   \tabitem Application logs, command execution results are encrypted （Section 5.2） \\ 
%     &~ & \tabitem Missing protection to the terminal (out of the scope)\\[.5\normalbaselineskip]
%     4.~File system and volume & Add STDIO shield in guest kernel   &   \tabitem Application logs, command execution results are encrypted （Section 5.2） \\ 
%     &~ & \tabitem Missing protection to the terminal (out of the scope)\\[.5\normalbaselineskip]
...
\begin{table}[H]
  ...
  \caption{Vulnerabilities in OCI runtime interface in the confidential computing and suggested solutions}
  \footnotesize
