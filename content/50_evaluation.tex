...
\begin{figure}[!htb]
    ...
    \subfloat[File type secrets are invisible on the host\label{fig:cquark_deployment_result_file_secret_mount_location}]{%
...
provisioning agent,  secret injector,  hardware evidence driver, and software measurement manager. Additionally, we use the statically linked hello world program from Listing~\ref{code2} as the test workload, which contains a global initialization array to regulate the binary data segment size. 
...
The baseline results in Figure~\ref{fig:application_start_microtest_baseline_time_overhead_each_cmp} show that the shielding layer doubled the program startup time. Notably, the remote attestation and provisioning agent contributed the most to the latency overhead, which is expected since it must 
...
Table~\ref{table:tcb_size_quark_vs_cquark}  summarizes the \acrshort{TCB} size in confidential and vanilla Quark. Vanilla Quark's guest kernel has 117615 lines of code and generates a statically linked binary of 3.10 MB. In confidential Quark, our implementation adds 4286 lines of code to the guest kernel binary, bringing the total 
